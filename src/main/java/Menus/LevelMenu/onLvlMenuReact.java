...
public class onLvlMenuReact extends ListenerAdapter {
    @Override
    public void onMessageReactionAdd(MessageReactionAddEvent e) {
        ...
        if(e.getReactionEmote().getName().equals("\u0030\u20E3")) {
            roleadd(target, "lvl 0 - 9", zero, gc, e.getUser(), list);
        }
        if (e.getReactionEmote().getName().equals("\u0031\u20E3")) {
            roleadd(target, "lvl 10-19", ten, gc, e.getUser(), list); //add 10 - 19
        } if(e.getReactionEmote().getName().equals("\u0032\u20E3")) {
                roleadd(target, "lvl 20-29", twenty, gc, e.getUser(),list); //add 20 - 29
            }
        if(e.getReactionEmote().getName().equals("\u0033\u20E3")) {
            roleadd(target, "lvl 30 - 39", thirty, gc, e.getUser(), list);
        }
        if (e.getReactionEmote().getName().equals("\u0034\u20E3")) {
            roleadd(target, "lvl 40 - 49", fourty, gc, e.getUser(), list);
        } if (e.getReactionEmote().getName().equals("\u0035\u20E3")) {
                roleadd(target, "lvl 50 - 59", fifty, gc, e.getUser(), list);
            } if (e.getReactionEmote().getName().equals("\u0036\u20E3")) {
                roleadd(target, "lvl 60 - 69", sixty, gc, e.getUser(), list);
            } if (e.getReactionEmote().getName().equals("\u0037\u20E3")) {
                roleadd(target, "lvl 70 - 79", seventy, gc, e.getUser(), list);
            } if (e.getReactionEmote().getName().equals("\u0038\u20E3")) {
                roleadd(target, "lvl 80 - 89", eighty, gc, e.getUser(), list);
            } if (e.getReactionEmote().getName().equals("\u0039\u20E3")) {
                roleadd(target, "lvl 90 - 104", ninety, gc, e.getUser(), list);
    ...
    private void roleadd(Member target, String srank, Role rrank, GuildController gc, User u, Role[] list) {
        gc.removeRolesFromMember(target, list).queue();
        gc.addSingleRoleToMember(target, rrank).queue();
        u.openPrivateChannel().queue(privateChannel -> {
            privateChannel.sendMessage(target.getAsMention() + ", you have been assigned the '"+srank+"' role.").queue();
        });
    }
}
